...
- **Chapters as first-class citizen** — Phases 1-2 built on branch `feature/chapters-first-class` (`Launch/plans/chapters-first-class.md`), STOPPED at the plan's checkpoint. New core semantic implemented: a chapter is a contiguous run of scenes in braid order, derived at read time by `src/shared/chapterSpans.ts` (`deriveChapterSpans`, unit tested), never a bulk DB rewrite. Rails view is now the full chapter management surface (create/rename/delete with merge-into-preceding, empty chapters as first-class spans positioned by `order`). Every braid-mutation call site in `App.tsx` (Rails + Braided List handlers) auto-reassigns a scene's chapter when it crosses a boundary. **Needs Brian to try Rails chapter UX in the real app before Phases 3-4 (Outline/Table/Editor/Compile consistency + empty-state polish) get built** — he's wary chapters may still feel unintuitive. **Known gap found during implementation, not yet resolved:** `BraidedListView` (List view, studio-build-only) and `PovOutlineView`/`TableView`/`OutlineView` still use the pre-redesign independent per-scene `chapterId` bucket model (including List view's own drag-to-reorder-chapters, which now contradicts the new "chapter order is derived, not draggable" rule) — flagged in `docs/features.md`, not touched this pass.
