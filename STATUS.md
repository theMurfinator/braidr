...
- **Chapters as first-class citizen** — Phases 0–2 BUILT and pushed (`feature/chapters-first-class`, not merged): span helper + tests, Rails chapter management (create/rename/delete-merges-down/empty chapters), auto-assignment on braid moves. Checkpoint verdict (2026-07-05): **data model validated; authoring surface rejected** — chapter-first writers need somewhere to start that isn't Rails. **BLOCKED on Brian's choice of authoring home**: dedicated Chapters screen (team rec) vs Editor-integrated nav vs Rails sidebar panel. Full checkpoint record + research findings in `Launch/plans/chapters-first-class.md`. Known gap: BraidedListView (studio-only) still has contradictory independent-buckets chapter UI, Phase 3 scope.
- **Data-model redesign** — Phases 0-6 done (PRs #61-#64). Phase 7 (zustand store — Brian taking this as part of the UI redesign) and Phase 8 (branch rework, last) remain. **Branch rework design is now LOCKED** (2026-07-05, Brian's UX answers): overlay model, not mutation-log forks — see `docs/data-model/BRANCHES.md`. Phase 8 stays sequenced last; write its implementation plan from that doc when it's next up.
...
- **Tags system removal scope** (2026-07-05) — running notes said "audit and rip out tags," but it's a real 5-table SQLite schema feature (`tags` + `scene_tags`/`note_tags`/`world_event_tags`/`task_tags` junction tables), branch-aware mutations with undo (`tag.upsert`/`tag.setCategory`/`tag.delete`), the importer, and ~20 UI components. This is the People/Locations/Arcs/Things/Time tag system from the original CLAUDE.md design doc, not a small add-on — removing it is migration-scale work, not a same-session bug fix, and it likely interacts with the still-pending Phase 8 branch rework. Needs a scoping conversation (all of it, or a specific surface like inline `#hashtags`?) before starting, not folded into a bug-fix batch.

## Recently shipped (context only — drop entries after a session or two)

- **Outline mode: 4 quick-win features** (2026-07-05, committed + pushed on `feature/braided-counts-notes-polish`, NOT merged/released) — scene titles editable inline (manuscript view + metadata panel); scroll position persists across view switches; "Go to scene" filter/jump box in the scene nav; Start/Stop timer moved into the meta panel per Brian ("hard no" on it living in the passage header). Brian manually verified in the dev app.
- **Tasks: subtask text wrapping fixed** (2026-07-05, committed + pushed, not merged) — subtask title editing now uses an auto-growing textarea instead of a single-line `<input>` (which can't wrap by definition); widened the subtask row indent.
- **Mood check-in feature fully removed** (2026-07-05, committed + pushed, not merged) — toolbar button, settings-menu entry, modal, custom category management, and the "Check-in Averages" analytics card are gone, per Brian's call. Also removed a dead automatic post-session check-in code path that no longer had any trigger left.
- **Branch feature rework DESIGNED, not built** (2026-07-05) — overlay model locked via Brian's UX answers, `docs/data-model/BRANCHES.md`; build stays sequenced as data-model Phase 8 (last).
